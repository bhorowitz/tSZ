double precision function integrand(lnM,z)
  USE cosmo
  USE sigma
  USE growth
...
! December 14, 2010: E.Komatsu
  ...
  double precision :: xin=1d-2,xout=6d0 ! pressure profile is cut at r=xout*r500
  double precision :: omega,tol=1d-6,zin=30d0,pi=3.1415926535d0
  double precision :: mvir,rvir,rs,delc,m500,r500,l500
  double precision :: rhoc,Ez,da,dvdz,transform2d,transform2dg,tsz,pgnfw,rombint1,hsexp,rombint2
  ...
  integer :: i
  external pgnfw,rombint1,hsexp,rombint2
  mvir=dexp(lnM)
...
! calculate the Fourier transform of Tsz
  ...
  transform2dg=1.65d0*(h0/0.7d0)**2d0*Ez(z)**(8d0/3d0) &
       *(m500/3d14/0.7d0)**(2d0/3d0+0.12d0) &
       *rombint2(hsexp,xin,xout,tol,ell/l500,rvir) &
       /0.5176d0*(4d0*3.14159d0)/l500**2d0*(r500/h0)

  Tsz=-2d0*283d0*((transform2d/50d0)*mgas(lnM,mcrit)+(1-mgas(lnM,mcrit))*(transform2dg/50d0)) ! uK, Rayleigh-Jeans limit
...
! This is the integrand: dV/dz*dn/dlnM*Tsz^2, to be integrated over lnM and z
  integrand=dvdz*dndlnMh*Tsz**2d0*mgas(lnM,mcrit)
  return
...
! December 14, 2010: E.Komatsu
  IMPLICIT none
  double precision, intent(IN) :: lnM,z
  double precision :: xin=1d-3,xout=6d0 ! pressure profile is cut at r=xout*r500
  double precision :: omega,tol=1d-6,zin=30d0,pi=3.1415926535d0
  double precision :: mvir,rvir,rs,delc,m500,r500,l500
...
! calculate the Fourier transform of Tsz
 ...
 ! print*, 10d0
  transform2d=1.65d0*(h0/0.7d0)**2d0*Ez(z)**(8d0/3d0) &
       *(m500/3d14/0.7d0)**(2d0/3d0+0.12d0) &
       *rombint1(pgnfw,xin,xout,tol,ell/l500,rvir) &
       /0.5176d0*(4d0*3.14159d0)/l500**2d0*(r500/h0)
...
! This is the integrand2: dV/dz*dn/dlnM*Tsz^2, to be integrated over lnM and z
  ...
  integrand2=dndlnMh*Tsz*bias(lnnu)
...
!-----------------------------------------------------------------------------
DOUBLE PRECISION FUNCTION hsexp(x,y,rvir) ! x=r/r500 & y=l/l500
  USE cosmo
  IMPLICIT none
  double precision :: x,y,rvir
  double precision :: coeff
  !coeff = 0.3938d0/(2.0d0*rvir)*rvir**1.0d0
  !coeff = ABS(4.77d0/(1453.96d0+5322.39d0 *(rvir-0.5d0)-2960.6d0* (rvir-0.5d0)**2d0-11044.0d0* (rvir-0.5d0)**3d0))/4.0d0
!3.669d0/(15.7496d0+94.4977d0* (rvir-0.5d0)+188.995d0* (rvir-0.5d0)**2.0d0+125.997d0* (rvir-0.5d0)**3.0d0)
  coeff = ABS(4.77d0/(28916.7d0+9537.66d0 *(rvir-0.5d0)-26835.9d0 *(rvir-0.5d0)**2+65875.1d0* (rvir-0.5d0)**3d0))
  !print*,rvir 
  hsexp=exp(-1.0d0*x**2.0d0/(2.0d0*(40.0d0*rvir)**2.0d0))*dsin(y*x)/(y*x)*coeff
